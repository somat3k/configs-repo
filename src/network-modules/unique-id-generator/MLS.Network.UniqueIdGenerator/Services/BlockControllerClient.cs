...
/// <summary>
/// Hosted background service that registers the unique-id-generator module with Block Controller
/// on startup and sends <c>MODULE_HEARTBEAT</c> every 5 seconds.
/// </summary>
public sealed class BlockControllerClient(
    HttpClient _http,
    IOptions<UniqueIdGeneratorConfig> _config,
    ILogger<BlockControllerClient> _logger) : BackgroundService
{
    ...
    private async Task RegisterAsync(CancellationToken ct)
    {
        ...
        var request = new
        {
            module_name   = UniqueIdGeneratorConstants.ModuleName,
            endpoint_http = opts.HttpEndpoint,
            endpoint_ws   = opts.WsEndpoint,
            capabilities  = new[] { "uuid-generation", "sequential-id-generation" },
            version       = "1.0.0",
        };
        try
        {
            ...
            if (reg is not null)
            {
                ...
                _logger.LogInformation(
                    "UniqueIdGenerator registered with Block Controller as {Id}", _registeredId);
            }
        }
    ...
    private async Task DeregisterAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("UniqueIdGenerator deregistered from Block Controller");
        }
        catch (Exception ex)
        {
