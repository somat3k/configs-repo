...
/// <summary>
/// Hosted background service that registers the runtime module with Block Controller.
/// </summary>
    ...
    ILogger<BlockControllerClient> _logger) : BackgroundService
{
    ...
    private async Task RegisterAsync(CancellationToken ct)
    {
        ...
        var request = new
        {
            module_name   = RuntimeConstants.ModuleName,
            endpoint_http = opts.HttpEndpoint,
            endpoint_ws   = opts.WsEndpoint,
            capabilities  = new[] { "module-lifecycle", "docker-management", "log-streaming" },
            version       = "1.0.0",
        };
        try
        {
            ...
            if (reg is not null)
            {
                ...
                _logger.LogInformation("Runtime registered with Block Controller as {Id}", _registeredId);
            }
        }
    ...
    private async Task DeregisterAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Runtime deregistered from Block Controller");
        }
        catch (Exception ex)
        {
