...
/// <summary>
/// Hosted background service that registers the subscription-manager module with Block Controller.
/// </summary>
public sealed class BlockControllerClient(
    HttpClient _http,
    IOptions<SubscriptionManagerConfig> _config,
    ILogger<BlockControllerClient> _logger) : BackgroundService
{
    ...
    private async Task RegisterAsync(CancellationToken ct)
    {
        ...
        var request = new
        {
            module_name   = SubscriptionManagerConstants.ModuleName,
            endpoint_http = opts.HttpEndpoint,
            endpoint_ws   = opts.WsEndpoint,
            capabilities  = new[] { "topic-subscription", "message-publishing", "pub-sub" },
            version       = "1.0.0",
        };
        try
        {
            ...
            var reg = await response.Content
                .ReadFromJsonAsync<RegistrationResponse>(ct).ConfigureAwait(false);
            if (reg is not null)
            {
                _registeredId = reg.ModuleId;
                _logger.LogInformation(
                    "SubscriptionManager registered with Block Controller as {Id}", _registeredId);
            }
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogWarning(ex, "Could not register with Block Controller — will retry");
        }
    }
    ...
    private async Task DeregisterAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("SubscriptionManager deregistered from Block Controller");
        }
        catch (Exception ex)
        {
