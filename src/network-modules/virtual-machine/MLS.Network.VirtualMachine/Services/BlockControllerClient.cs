...
/// <summary>
/// Hosted background service that registers the virtual-machine module with Block Controller.
/// </summary>
public sealed class BlockControllerClient(
    HttpClient _http,
    IOptions<VirtualMachineConfig> _config,
    ILogger<BlockControllerClient> _logger) : BackgroundService
{
    ...
    private async Task RegisterAsync(CancellationToken ct)
    {
        ...
        var request = new
        {
            module_name   = VirtualMachineConstants.ModuleName,
            endpoint_http = opts.HttpEndpoint,
            endpoint_ws   = opts.WsEndpoint,
            capabilities  = new[] { "strategy-execution", "sandbox-isolation", "script-evaluation" },
            version       = "1.0.0",
        };
        try
        {
            ...
            if (reg is not null)
            {
                ...
                _logger.LogInformation(
                    "VirtualMachine registered with Block Controller as {Id}", _registeredId);
            }
        }
    ...
    private async Task DeregisterAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("VirtualMachine deregistered from Block Controller");
        }
        catch (Exception ex)
        {
