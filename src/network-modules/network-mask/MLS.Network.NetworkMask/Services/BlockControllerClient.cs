...
/// <summary>
/// Hosted background service that registers the network-mask module with Block Controller.
/// </summary>
    ...
    ILogger<BlockControllerClient> _logger) : BackgroundService
{
    ...
    private async Task RegisterAsync(CancellationToken ct)
    {
        ...
        var request = new
        {
            module_name   = NetworkMaskConstants.ModuleName,
            endpoint_http = opts.HttpEndpoint,
            endpoint_ws   = opts.WsEndpoint,
            capabilities  = new[] { "endpoint-registry", "url-resolution", "environment-routing" },
            version       = "1.0.0",
        };
        try
        {
            ...
            if (reg is not null)
            {
                ...
                _logger.LogInformation(
                    "NetworkMask registered with Block Controller as {Id}", _registeredId);
            }
        }
    ...
    private async Task DeregisterAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("NetworkMask deregistered from Block Controller");
        }
        catch (Exception ex)
        {
