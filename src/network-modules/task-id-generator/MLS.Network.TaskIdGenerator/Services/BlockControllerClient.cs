...
/// <summary>
/// Hosted background service that registers the task-id-generator module with Block Controller
/// on startup and sends <c>MODULE_HEARTBEAT</c> every 5 seconds.
/// </summary>
public sealed class BlockControllerClient(
    HttpClient _http,
    IOptions<TaskIdGeneratorConfig> _config,
    ILogger<BlockControllerClient> _logger) : BackgroundService
{
    ...
    private async Task RegisterAsync(CancellationToken ct)
    {
        ...
        var request = new
        {
            module_name   = TaskIdGeneratorConstants.ModuleName,
            endpoint_http = opts.HttpEndpoint,
            endpoint_ws   = opts.WsEndpoint,
            capabilities  = new[] { "task-id-generation", "task-id-validation" },
            version       = "1.0.0",
        };
        try
        {
            ...
            if (reg is not null)
            {
                ...
                _logger.LogInformation(
                    "TaskIdGenerator registered with Block Controller as {Id}", _registeredId);
            }
        }
    ...
    private async Task DeregisterAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("TaskIdGenerator deregistered from Block Controller");
        }
        catch (Exception ex)
        {
