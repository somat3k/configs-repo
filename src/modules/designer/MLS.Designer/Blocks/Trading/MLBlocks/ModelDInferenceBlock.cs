...
/// <summary>
/// ML Inference block for the DeFi model (model-d).
/// Sends a feature vector to ML Runtime and emits a
/// <see cref="BlockSocketType.DeFiSignal"/> with yield prediction.
/// </summary>
public sealed class ModelDInferenceBlock : BlockBase
{
    ...
    [
        BlockSocket.Output("ml_output", BlockSocketType.DeFiSignal),
    ];

    private readonly HttpClient _http;

    private readonly BlockParameter<string> _modelIdParam =
        new("ModelId",             "Model ID",             "ML Runtime model identifier", "model-d");
    private readonly BlockParameter<float>  _confidenceParam =
        new("ConfidenceThreshold", "Confidence Threshold", "Minimum confidence to emit",  0.70f, MinValue: 0f, MaxValue: 1f, IsOptimizable: true);
    ...
    /// <inheritdoc/>
    public override string BlockType   => "ModelDInferenceBlock";
    /// <inheritdoc/>
    public override string DisplayName => "Model-D Inference";
    /// <inheritdoc/>
    public override IReadOnlyList<BlockParameter> Parameters => [_modelIdParam, _confidenceParam, _timeoutMsParam];

    /// <summary>Initialises a new <see cref="ModelDInferenceBlock"/> with the injected HTTP client.</summary>
    public ModelDInferenceBlock(HttpClient http) : base(_inputs, _outputs) => _http = http;

    /// <summary>Default constructor for <see cref="Services.BlockRegistry"/>.</summary>
    public ModelDInferenceBlock() : this(new HttpClient()) { }
    ...
    protected override async ValueTask<BlockSignal?> ProcessCoreAsync(BlockSignal signal, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var defiSignal = new { action = result.Class, confidence = result.Confidence, model_name = result.ModelName };
            return EmitObject(BlockId, "ml_output", BlockSocketType.DeFiSignal, defiSignal);
        }
